...
# CONSTRAINTS: STANDALONE VLAN INTERFACE (RouterOS 7)

This covers the standalone 802.1Q VLAN sub-interface feature
(`/interface vlan`), NOT bridge VLAN filtering (`/interface bridge vlan`,
see the `bridge-vlan` topic instead). Use this when the goal is a single
tagged VLAN sub-interface on a parent interface, not a multi-port bridge
trunk/access setup.
